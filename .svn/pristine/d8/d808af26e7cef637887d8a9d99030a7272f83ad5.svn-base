...
{
    if ($set) {
        $comp = Company::find(session()->get('comp_id', 1));
        $db = config('database.connections.mysql_comp');
        $db['database'] = config('database.data_name') . $comp->shared_comp_id . '_' . 'company';
        // Log::info($db);
...
function getYearlyDbConn($set = false)
{
    if ($set) {
        ...
        $db['database'] = config('database.data_name') . session()->get('comp_id', 1) . '_' . getFY();
        // Log::info($db);
